...
- **Custom CSS** (Interface → Advanced) — loads `~/.orca/custom.css` after the built-in theme and reapplies it every time you save the file, so palette generators like pywal or matugen can keep Orca in sync. Override the color variables under `:root` (light) and `.dark` (dark) — e.g. `--background`, `--foreground`, `--border`, and `--worktree-sidebar` for the left sidebar; the file is created with a commented starting point the first time you open it. Remote `url()` and `@import` are ignored, and the variable names are internal, so they can change between releases. **Match terminal** / **Tinted** sidebar modes still set the sidebar colors; use **Default** (or `!important`) to let the file control them. Plugin panels keep the colors they opened with, and if a rule ever hides the settings screen, empty or delete the file to recover.
